    ...
    public partial class TilemapEditor : Editor
    {
        ...
        private void OnInspectorGUI_Renderer()
        {
            ...
            */
            ...
            {
                string path = EditorUtility.SaveFilePanel("Save tilemap Png", AssetDatabase.GetAssetPath(m_tilemap), m_tilemap.name + ".png", "png");
                if(!string.IsNullOrEmpty(path))
                {                    
                    Texture2D tilemapText = TilemapUtilsEditor.CreateTexture2DFromTilemap(m_tilemap);
                    System.IO.File.WriteAllBytes(path, tilemapText.EncodeToPNG());
                }
            }
            EditorGUILayout.Space();
